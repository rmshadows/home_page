...
def getPage(sample_html, json_conf):
    ...
    li_b = []
    li_c = []
    li_len = len(json_conf['links'])
    print("一共有{}个友链".format(li_len))
    nc = 1
    for il in json_conf['links']:
        if nc == 1:
            li_a.append(il)
            nc += 1
        elif nc == 2:
    ...
    with open(sample_html+".html", "r", encoding="utf-8") as f:
        ...
        for line in f.readlines():
            ...
            if "{^START" in line:
                # 清空临时列表
                tmp_par = []
                if line.replace("{^START", "").replace("^}", "").replace(" ", "").replace("\t", "") == "A":
                    list_flag = 1
                elif line.replace("{^START", "").replace("^}", "").replace(" ", "").replace("\t", "") == "B":
                    list_flag = 2
                elif line.replace("{^START", "").replace("^}", "").replace(" ", "").replace("\t", "") == "C":
                    list_flag = 3
                else:
                    list_flag = -1
                    print("list_flag == -1 !")
                ...
                if list_flag == 1:
                    ...
                    print("list_flag({}) not in (1,2,3)".format(list_flag))
                    sys.exit(1)
                # 获取json 每个json
                for j in links_list:
                    # 每个循环段
...
if __name__ == '__main__':
    ...
    home_page_conf = loadConfJSON("links.conf")
